...
const DesignPreview = ({ configuration }: DesignPreviewProps) => {
  ...
  const tw: string | undefined = COLORS.find((supportedColor) => supportedColor.value === color)?.tw;
  const { label: modelLabel } = MODELS.options.find(({ value }) => value === model)!;

  let totalPrice = BASE_PRICE; // Prices in cents
  ...
  const handleCheckout = async () => {
    if (!user) {
      localStorage.setItem(
        'configurationData',
        JSON.stringify({ ...configuration, totalPrice })
      );
    ...
    if (paymentMethod === 'cash') {
      ...
      if (result.ok) {
        ...
        toast({
          title: 'Order failed',
          description: 'Failed to create cash order. Please try again.',
          variant: 'destructive',
        });
      }
    } else {
    ...
    <>
      ...
      <div className="mt-20 flex flex-col items-center md:grid text-sm sm:grid-cols-12 sm:grid-rows-1 sm:gap-x-6 md:gap-x-8 lg:gap-x-12">
        <div className="md:col-span-4 lg:col-span-3 md:row-span-2 md:row-end-2">
          <Phone
            className={cn(tw ? `bg-${tw}` : '', 'max-w-[150px] md:max-w-full')}
